...
public class CameraLine extends View {
    ...
    private void getAttrsAndInit(Context context, AttributeSet attrs) {
        ...
        if (lineCrossLength != 0) {
            crossLineLength = CameraUtils.dip2px(getContext(), lineCrossLength);
        }
        if (lineCrossWidth != 0) {
            mLineCrossPaint.setStrokeWidth(CameraUtils.dip2px(getContext(), 1));
        }
        if (lineCrossColor != 0) {
    ...
    private void init() {
        ...
        mLinePaint.setStrokeWidth(CameraUtils.dip2px(getContext(), 1));//默认1dp


        mLineCrossPaint = new Paint();
        mLineCrossPaint.setColor(Color.parseColor("#55000000"));//00-FF
        mLineCrossPaint.setStrokeWidth(CameraUtils.dip2px(getContext(), 1));//默认1dp
