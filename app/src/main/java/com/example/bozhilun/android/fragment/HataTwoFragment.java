...
public class HataTwoFragment extends BaseFragment {
    @BindView(R.id.chart_step_shuimia_xinlvtwo) ColumnChartView shuimianjilu;
    @BindView(R.id.yungdongstep_shangzhou_xinlvtwo) TextView shangzhou;
    @BindView(R.id.yungdongstep_benzhou_xinlvtwo) TextView benzhou;

    @BindView(R.id.stepval_tv_shuimianone_xinlvtwo) TextView quantian;
    @BindView(R.id.activityval_tv_shuimianone_xinlvtwo) TextView Shengshuimian;
    @BindView(R.id.lichengval_tv_shuimianone_xinlvtwo) TextView Qianshuimian;
                        ...
                        try{
                            ...
                            if(null!=dataActivityReport.getHeartRate()){
                                ...
                                for (int i = 0; i < 7; ++i) {
                                    ...
                                    for (int j = 0; j < sportHours.size(); ++j) {
                                        ...
                                        axisValuesY.add(new AxisValue(MaxHeartRate).setValue(MaxHeartRate));// 添加Y轴显示的刻度值
                                        axisValuesY.add(new AxisValue(minHeart).setValue(minHeart));// 添加Y轴显示的刻度值
                            ...
                            /*        Column column2 = new Column(values2);
                               ...
                               stepColumdata.setValueLabelBackgroundAuto(false);// 设置数据背景是否跟随节点颜色
                                ...
                                String avgHeartRate = jsonObject.getString("max_min");
                                JSONObject AAA=new JSONObject(avgHeartRate);
                                if(!"null".equals(AAA.getString("maxHeartRate").toString())){
                                    quantian.setText(AAA.getString("maxHeartRate").toString());
                                   Qianshuimian.setText(AAA.getString("minHeartRate").toString());
                                    Shengshuimian.setText((Integer.valueOf(AAA.getString("maxHeartRate").toString())+Integer.valueOf(AAA.getString("minHeartRate").toString()))/2+"");
                                }else{
                                    quantian.setText("- -");
    ...
    @OnClick({R.id.yungdongstep_benzhou_xinlvtwo, R.id.yungdongstep_shangzhou_xinlvtwo})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.yungdongstep_benzhou_xinlvtwo:
                shangzhou.setTextColor(Color.parseColor("#575757"));
                benzhou.setTextColor(Color.parseColor("#000000"));
                sdf = new SimpleDateFormat("yyyy-MM-dd");
                getData(EndTiem,sdf.format(new Date()).toString());
                break;
            case R.id.yungdongstep_shangzhou_xinlvtwo:
    ...
    protected int getContentViewId() {
        return R.layout.fragment_hatatwo;
    }
    ...
    private void getData(String starttime,String endtime) {
        ...
        OkHttpObservable.getInstance().getData(commonSubscriber, URLs.HTTPs+URLs.getHeartRateW, mapjson);
    }
