...
public class XieyaTwoFragment extends BaseFragment {
    @BindView(R.id.chart_step_shuimia_xieyatwo) ColumnChartView shuimianjilu;
    @BindView(R.id.yungdongstep_shangzhou_xieyatwo) TextView shangzhou;
    @BindView(R.id.yungdongstxieyatwo) TextView benzhou;
                            ...
                            if(null!=dataActivityReport.getBloodPressure()){
                                ...
                                for (int i = 0; i < 7; ++i) {
                                    ...
                                    for (int j = 0; j < sportHours.size(); ++j) {
                                        ...
                                        MaxHeartRate = sportHours.get(j).getMinSystolic();
                                        minHeart= sportHours.get(j).getMaxDiastolic();
    ...
    @OnClick({R.id.yungdongstxieyatwo, R.id.yungdongstep_shangzhou_xieyatwo})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.yungdongstxieyatwo:
                shangzhou.setTextColor(Color.parseColor("#575757"));
                benzhou.setTextColor(Color.parseColor("#000000"));
                sdf = new SimpleDateFormat("yyyy-MM-dd");
                getData(EndTiem,sdf.format(new Date()).toString());
                break;
            case R.id.yungdongstep_shangzhou_xieyatwo:
    ...
    protected int getContentViewId() {
        return R.layout.fragment_xieyatwo;
    }
    ...
    private void getData(String starttime,String endtime) {
        ...
        OkHttpObservable.getInstance().getData(commonSubscriber, URLs.HTTPs+URLs.getBloodPressureW, mapjson);
    }
