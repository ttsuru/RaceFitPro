...
//            bottomPadding = ta.getDimension(R.styleable.AbsLeafChart_lc_bottomPadding, LeafUtil.dp2px(mContext, 20));
    ...
    protected void resetAsixY() {
        if (axisY != null) {
            List<B18iAxisValue> values = axisY.getValues();
            ...
            for (int i = 0; i < sizeY; i++) {
                ...
                b18iAxisValue.setPointX(leftPadding);
                if (i == 0) {
                    b18iAxisValue.setPointY(mHeight - bottomPadding - startMarginY);
                } else {
                    b18iAxisValue.setPointY(mHeight - bottomPadding - startMarginY - yStep * i);
                }
            }
    ...
    protected void resetAsixX() {
        if (axisX != null) {
            List<B18iAxisValue> values = axisX.getValues();
            int sizeX = values.size(); //几条y轴
            float xStep = (mWidth - leftPadding - startMarginX) / sizeX;
            for (int i = 0; i < sizeX; i++) {
                B18iAxisValue b18iAxisValue = values.get(i);
                b18iAxisValue.setPointY(mHeight);
                if (i == 0) {
                    b18iAxisValue.setPointX(leftPadding + startMarginX);
                } else {
                    b18iAxisValue.setPointX(leftPadding + startMarginX + xStep * i);
                }
            }
...
//            axisX.setStartY(mHeight - bottomPadding).setStopX(mWidth).setStopY(mHeight - bottomPadding);
    ...
    protected void resetPointWeight(ChartData chartData) {
        if (chartData != null && axisX != null && axisY != null) {
            ...
            List<B18iAxisValue> b18iAxisValuesX = axisX.getValues();
            List<B18iAxisValue> b18iAxisValuesY = axisY.getValues();
            float totalWidth = Math.abs(b18iAxisValuesX.get(0).getPointX() - b18iAxisValuesX.get(b18iAxisValuesX.size() - 1).getPointX());

            float totalHeight = Math.abs(b18iAxisValuesY.get(0).getPointY() - b18iAxisValuesY.get(b18iAxisValuesY.size() - 1).getPointY());
