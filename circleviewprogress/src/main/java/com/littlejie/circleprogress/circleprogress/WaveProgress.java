...
public class WaveProgress extends View {
    ...
    //绘制目标步数
    private TextPaint setTagPaint;
    private String tagStepStr = "";
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mMaxValue = typedArray.getFloat(R.styleable.WaveProgress_maxValue, getMaxValue());
    ...
    private void initPaint() {
        ...
        //绘制步数
        setTagPaint = new TextPaint();
        setTagPaint.setAntiAlias(antiAlias); //抗锯齿
        setTagPaint.setTextSize(30);
        setTagPaint.setColor(Color.WHITE);
        setTagPaint.setTextAlign(Paint.Align.CENTER);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Log.e(TAG, "----onSizeChanged: 控件大小 = " + "(" + getMeasuredWidth() + ", " + getMeasuredHeight() + ")"
    ...
    private void drawProgress(Canvas canvas) {
       // Log.e(TAG,"-----drawProgress--");
        float y = mCenterPoint.y - (mPercentPaint.descent() + mPercentPaint.ascent()) / 2;
        canvas.drawText(String.format("%.0f", mPercent * mMaxValue), mCenterPoint.x, y, mPercentPaint); //绘制数字+getResources().getString(R.string.steps)
        //绘制显示的步    //(String.format("%.f", mPercent * mMaxValue))
        canvas.drawText(getResources().getString(R.string.steps), mCenterPoint.x+(mPercentPaint.measureText(String.format(Locale.US,"%s", decimalFormat.format(mValue))))/2+50, y, mHintPaint);
        //绘制目标步数
        canvas.drawText(getTagStepStr(),mCenterPoint.x,(mCenterPoint.y +(mCenterPoint.y / 2)),setTagPaint);
        //Log.e(TAG,"---------mValue-="+mValue+"--=="+mPercent * mMaxValue+"---="+decimalFormat.format(mValue));
    }
    ...
    public void setTagStepStr(String tagStepStr) {
        this.tagStepStr = tagStepStr;
    }
